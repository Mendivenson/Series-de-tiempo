...
Para el ajuste del suavizamiento exponencial tomaremos las series originales (O transformadas para la estabilización de la varianza) para revisar si el método estima la tendencia y la estacionalidad del proceso.

### **Serie mensual:** Volumen de leche captada en el país. {.active}

Se plantearán dos modelos:

- $a_{t} = \alpha x_t + (1 - \alpha) a_{t-1}$
- $\alpha$
...
```{python}
# En R se debe instalar el paquete reticulate
